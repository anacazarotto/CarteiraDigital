...
namespace CarteiraDigital.Repositories
{
    public class InflowRepository
    {
        ...
        public List<Inflow> SearchFilter(Filter filter)
        {
            ...
            p.InflowDate <= (filter.Period > 0 ? DateTime.Now : filter.MaxDate != DateTime.MinValue ? filter.MaxDate : DateTime.MaxValue)
            );
       ...
       public double SumAmount()
        {
            return _session.Query<Inflow>().Sum(i => (double?)i.InflowAmount) ?? 0;
        }
